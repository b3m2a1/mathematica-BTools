...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated NewApp loader file *)


Unprotect[NewApp`Private`Package`$TopLevelLoad];
NewApp`Private`Package`$TopLevelLoad=
...
$PackageContexts={
		"NewApp`",
		"NewApp`Private`Package`",
		"NewApp`Private`Hidden`"
		};
$PackageDeclared=
...
PackageFileContext[f_String?DirectoryQ]:=
	With[{s=PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s,NewApp],"`"]
		];
...
PackageScopeBlock[e_,scope_String:"Hidden"]/;TrueQ[$AllowPackageRescoping]:=
	With[{newcont="NewApp`Private`"<>StringTrim[scope,"`"]<>"`"},
...
PackageDecontext[
	...
	With[{
		...
		ctx="NewApp`Private`"<>StringTrim[scope,"`"]<>"`"
		},
		Replace[names,
...
If[(Clear@NewApp`Private`Package`$loadAbort;!#)&@NewApp`Private`Package`$loadAbort,
	NewApp`Private`Package`PackagePostProcessContextPathReassign[]
	]