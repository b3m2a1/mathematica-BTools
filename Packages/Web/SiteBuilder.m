...
WebSiteDeploy::usage="Deploys a website";
WebSiteGitHubDeploy::usage="Deploys a directory to GitHub Pages";
WebSiteCloudDeploy::usage="Deploys a directory to the Wolfram Cloud";
...
(* ::Text:: *)
(*
	Originally only had Wolfram Cloud deployment but I also need to support GitHub Pages since the WC
	is no longer a viable free option
*)



WebSiteCloudDeploy::depfail="Failed to deploy `` to ``";
...
getWebsiteDeployDir[outDir_, opd_]:=
  With[{
    ext=
      Replace[
        opd,
...
webSiteDeployFile[f_, uri_, outDir_, trueDir_, stripDirs_, ops___?OptionQ]:=
  Module[{
    ...
    perm=
      Lookup[
        Flatten@{ops, Options[WebSiteCloudDeploy]},
        Permissions,
        "Private"
        ];
    ...
    Replace[
      {
        ...
        e_:>(Message[WebSiteCloudDeploy::depfail, f, url];e)
        }
      ]@
      CopyFile[
        f,
...
webSiteDeploy[
  ...
  last_, select_,
  ops:OptionsPattern[WebSiteCloudDeploy]]:= 
  (
...
WebSiteCloudDeploy[
  ...
  Module[
    {
      trueDir=getWebsiteDeployDir[outDir, OptionValue["OutputDirectory"]],
      info,
      select,
      last
      },
...
WebSiteCloudDeploy[
  ...
  WebSiteCloudDeploy[outDir, uri, Normal[ops]];
...
getGHDeployRoot[outDir_]:=
  If[MemberQ[{"out", "output", "docs"}, FileBaseName[outDir]],
...
getDeploymentSettings[dir_, op:OptionsPattern[]]:=
  Module[
    {
      ops = Quiet@Association@Import[FileNameJoin@{dir, "SiteConfig.wl"}]
      },
    Normal@
      If[!AssociationQ[ops],
        {},
        Lookup[ops, "DeployOptions", {}]
        ]
    ]
...
$deploymentDispatcher=<|
  "Cloud"->WebSiteCloudDeploy,
  "GitHub"->WebSiteGitHubDeploy
  |>;
...
WebSiteDeploy[
  ...
  PackageExceptionBlock["WebSiteDeploy"]@
    Module[
      {
        out = getWebsiteDir[outDir],
        settings,
        depTarg,
        disp
        },
      settings = getDeploymentSettings[out];
      depTarg = 
        Replace[
          OptionValue[WebSiteDeploy, 
            FilterRules[{ops}, Options[WebSiteDeploy]], 
            "DeploymentTarget"
            ], 
          Automatic:>Lookup[settings, "DeploymentTarget", "GitHub"]
          ];
      disp = $deploymentDispatcher[depTarg];
      If[MissingQ[disp],
        PackageRaiseException[
          Automatic,
          "Site deployment target `` unknown. Known targets are ``",
          Keys[$deploymentDispatcher]
          ],
        disp[
          out,
          uri,
          FilterRules[
            Normal@Merge[{settings, ops}, Last],
            Options[disp]
            ]
          ]
        ]
