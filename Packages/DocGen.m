...
DocRefLink::usage="Generates RefLink boxes to a symbol";
...
If[MatchQ[$DocLinkBase,{Except[_Rule]..}|Except[_List]],
	$DocLinkBase={
		"System"->
			Nothing
		};
...
pacletLinkBuild[s_,context_,type_]:=
	StringJoin@{"paclet:",
		Riffle[Flatten@{Replace[context,"System"->Nothing],type,s},"/"]
		};
pacletLinkBuild[s_,type_]:=
	If[MatchQ[s,_String?(StringMatchQ["paclet:*"])],
		s,
		With[{base=
			Replace[s,{
				Except[_String]:>
...
docSymType[sym_Symbol]:=
	Replace[StringTrim[Context@sym,"`"],
		"System"->"BUILT-IN SYMBOL"];
docSymType~SetAttributes~HoldFirst;
docTypeColor[s_]:=
	s/.Append[$DocumentationColoring,_->RGBColor[0.023529, 0.73, 0.729412]]
...
docLinkType[sym_Symbol]:=
	Replace[DocLinkBase[sym],
		...
		Append[$DocumentationColoring,
			_->"PackageLink"];
docLinkType~SetAttributes~HoldFirst;


DocRefLink[s_String]:=
	Cell[BoxData@TemplateBox[{Cell[TextData[s]],pacletLinkBuild[s]},
		"RefLink",BaseStyle->{"InlineFormula"}]];
DocRefLink[s_Symbol]:=
	With[{sname=SymbolName@Unevaluated[s]},
		DocRefLink[sname]
		];
DocRefLink[h_Hyperlink]:=
	Cell[BoxData@ToBoxes@h,"Hyperlink"];
DocRefLink[e_?BoxQ]:=
	e/.s_String?makeRefTest:>DocRefLink[s];
DocRefLink~SetAttributes~HoldFirst
...
parseRefText[t_]:=
	ReplaceAll[
		...
		Cell[BoxData[FormBox[s_,_?(ToString[#]=="InlineRef"&)]],
			___]:>
			Replace[s,{
				_String?makeRefTest:>
					DocRefLink[s],
		...
		Cell@BoxData@
			DynamicBox[
				If[ $VersionNumber < 11.1,
					"",
					Cell[BoxData@ToBoxes@sectionsToggler[ids]]
					],
					...
					Grid[{{
						Item[
							Row@{
								Spacer[8],
								RawBoxes@
									Cell[docSymType@sym,"PacletNameCell",
...
iGenerateTitleCell[sym_]:=
	Cell[ToString@Unevaluated[sym], "ObjectName"];
...
generateUsageCell[symbol_Symbol,e_]:=
	generateUsageCell[Evaluate@ToString@Unevaluated[symbol],e];
...
generateBasicExample[
	...
	Cell[
		CellGroupData[{
			...
			Open]];
generateBasicExample[Cell[d_,"ExampleSubsection",e___]]:=
	openerCellGroup@{
		d,"ExampleSubsection",
		If[IntegerQ@$exampleCounter,ToString@$exampleCounter++,"0"],
		e
		};
		...
		openerCellGroup[{"Examples","PrimaryExamplesSection"},
			Map[
				openerCellGroup[{
					...
					Replace[Thread@Extract[#,{1,2},Hold],{
						Hold[Hold[e_]]:>generateBasicExample[e],
						Hold[e_]:>generateBasicExample[e]
						},
						1]
					]&,
			...
			If[Not@MemberQ[First/@exampleSections,"Options"]&&
				...
				openerCellGroup[{
					...
					openerCellGroup@{
						...
						ToString@$exampleCounter++}&/@Keys@Options[sym]
					],
				Nothing
				]
			]
...
GenerateRefPages[s_Symbol,ops:OptionsPattern[]]:=
	CheckAbort[
		...
		CreateDocument[
			...
			WindowTitle->(ToString@Unevaluated[s]<>" - Documentation"),
...
IndexDocumentation[
	...
	With[{
		...
		With[{ind=DocumentationSearch`NewRefPageNotebookIndexer[indexDir]},
			Map[
				DocumentationSearch`AddRefPageNotebook[ind,#]&,
				FileNames["*.nb",src,\[Infinity]]
				];
			DocumentationSearch`CloseRefPageNotebookIndexer[ind];
			];
