...
GenerateNewPackage::usage="Opens a new package file template";
...
BundleApp::usage="Creates a sync bunde for an app";
UploadApp::usage="Uploads an application zip to the cloud";
DownloadApp::usage="Downloads an app into a directory";
InstallApp::usage="Downloads/installs an application";
BackupApp::usage="Backs up the app";
AppBackups::usage="Gets all the backed-up versions of the app";
RestoreApp::usage="Restores the most recent version of the app";
...
AppDownloadForm::usage=
	"A form function that provides downloads for an app or server base";


AppPacletDownloadForm::usage=
	"A form function that provides paclet installers for an app or server base";
...
configurePackages[name_String,packages:(_String|_Symbol|{(_String|_Symbol)..})]:=
	With[{files=Cases[Replace[Hold[packages],Hold[{p__}]:>Hold[p]],
		_String?FileExistsQ
		],
		defs=getDefinitions@@Replace[Thread[
		...
		If[MatchQ[defs,_String],
			With[{defFile=OpenWrite@FileNameJoin@{$AppDirectoryRoot,$AppDirectoryName,
				name,"Packages","ContextPackage.m"}
				},
				WriteString[defFile,defs];
				Close@defFile;
				]
			];
		Do[CopyFile[f,FileNameJoin@{
			$AppDirectoryRoot,$AppDirectoryName,name,"Packages",FileNameTake@f},
...
configureDocs[name_String,
	docNotebooks:(Rule[_String,_Notebook]|_String?(StringMatchQ[#,"*.nb"]&))..]:=Do[
	With[{f=FileNameJoin@{$AppDirectoryRoot,$AppDirectoryName,
		"Name",
		"Documentation",
		"English",
			FileNameTake@
				If[MatchQ[d,_String],
					d,
					StringTrim[First@d,".nb"]<>".nb"
					]}},
		FileTemplateApply[If[MatchQ[d,_String],
		File@d,
		Last@d],
		<|"app"->name,
		"file"->f
...
ConfigureApp[name_String,
	packages:
		(_FileNameJoin|_String|_Symbol|{(_FileNameJoin|_String|_Symbol)...})|None:
		None,
	ops:OptionsPattern[]
	]:=
	Block[{$AppDirectoryRoot=
		Replace[OptionValue@Directory,Automatic:>$AppDirectoryRoot]},
		configureDirectories@name;
		Replace[
			Hold[packages]/.{
				None->{},
				f_FileNameJoin:>With[{r=f},r/;True]
				},
			Hold[p__]:>configurePackages[name,p]
			];
		AppRegenerateInit@name;
		configureDocs[name,Sequence@@Flatten@{OptionValue@"Documentation"}];
		AppRegeneratePacletInfo[name,Sequence@@Flatten@{OptionValue@"Paclet"}];
		FileNameJoin@{$AppDirectoryRoot,$AppDirectoryName,name}
		];
ConfigureApp~SetAttributes~HoldAll;
...
AppLoadPackage[appName_,heldSym_,pkgFile_->syms_]:=
	With[{app=AppFromFile[appName]},
		...
		BeginPackage[app<>"`"];
		AppendTo[
			$AppLoadedPackages[app],
			FileBaseName@pkgFile
			];
		Get[pkgFile];
		EndPackage[];
		ReleaseHold[heldSym]
		];
AppDeclarePackage[appName_,pkgFile_->syms_]:=
	Map[
		If[Not@MatchQ[Apply[OwnValues][#],{_:>_AppLoadPackage}],
			#:=AppLoadPackage[appName,#,pkgFile->syms];
			Replace[#,
				Verbatim[HoldPattern][s_]:>(
					s/:HoldPattern[
						m:Except[Clear|ClearAll|OwnValues|Set|SetDelayed][s,__]]:=
						(s;m)
					)]]&,
		syms
		];
AppLoadDeclare[appName_,pkgFile_String]:=
	With[{app=AppFromFile[appName]},
		If[Not@KeyMemberQ[$AppLoadedPackages,app],
			$AppLoadedPackages[app]={}
			];
		If[Not@MemberQ[$AppLoadedPackages[app],FileBaseName@pkgFile],
			With[{decs=AppPackageFunctions[pkgFile]},
				AppDeclarePackage[app,
					pkgFile->ToExpression[decs,StandardForm,HoldPattern]
					]
				],
			AppGet[app,FileBaseName@pkgFile]
			]
		]
...
$AppBackupDirectoryName="_Apps Cache"
...
InstallApp[
		...
		loadFE_:Automatic]:=
	With[{},
		If[!DirectoryQ@AppDirectory@name,DownloadApp@name];
		If[Replace[loadFE,
			Automatic:>(FileNames["*",{
...
DownloadApp[
	...
	DownloadApp[file,name,root,"DownloadFrom"->source,ops];
...
BundleApp[
	...
	Replace[OptionValue["BundleInfo"],{
		Automatic:>
			BundleApp[paths,exportDir,metadata,
				"BundleInfo"->
		...
		f:(_String|_File)?FileExistsQ|_URL:>
			BundleApp[paths,exportDir,metadata,
				...
				Replace[Import[f],{
					o:{__?OptionQ}:>
						(Sequence@@FilterRules[o,Options@BundleApp]),
					_:>(Sequence@@{})
					}],
				ops
				],
	...
	With[{dir=
		...
		UploadFile[
			If[FileExistsQ@appName,
				appName,
				BundleApp[appName,
					FilterRules[{ops},Options@BundleApp]]
				],
			$AppCloudExtension,
			ops
			]
