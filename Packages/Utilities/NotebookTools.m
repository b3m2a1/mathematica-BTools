...
PaletteTemplateNotebook::usage=
  "Generates a palette template notebook";
...
acButton[item_, paste_,ops:OptionsPattern[Button]]:=
Button[
item,
paste[item],
ops,
...
acPanel[items_, paste_,truncateAt:_Integer|None:10, ops:OptionsPattern[Panel]]:=
DynamicModule[
{
displayAll,
menuItems,
...
Append[
menuItems[[;;truncate]],
{acOCButton[Style["\[DownPointer]", Gray, Large], (truncate=Length@menuItems)&]}
],
...
Appearance->
{
"Default"->FrontEnd`FileName[{"Popups", "CodeCompletion"}, "top_left.9.png"]
}
],
Initialization:>{
menuItems=Transpose[{acdb[#, paste]&/@items}]
}
];
...
SummarizedForm[
  expr_,
  head:_String|Automatic:Automatic,
  graphic:Except[_List]:None,
  alwaysView:_List|_Function:{},
  expandedView:_List|_Function:{},
  template:_Association|_List|Automatic:Automatic
  ]:=
  With[{
    driver=
      Association@Replace[template,
        Automatic:>
          Replace[expr,{
            _[a_Association]:>a,
            _-><||>
    ...
    RawBoxes@
      BoxForm`ArrangeSummaryBox[
        Replace[head,Automatic:>ToString@Head[expr]],
        expr, 
        graphic,
        Replace[alwaysView,{
          f_Function:>
...
(*SummarizedForm~SetAttributes~HoldAll;*)
...
IconForm[
    expr_,
    iconExpr_,
    infoSpec:{(_List|_Rule|_RuleDelayed)...}:{},
    ops:OptionsPattern[]]:=
  With[{
    icon=
    Replace[iconExpr,
      {
        i_Image:>Image[i,ImageSize->OptionValue@ImageSize],
        g:_Graphics|_Graphics3D:>Show[g,ImageSize->OptionValue@ImageSize],
        e:Except[None]:>Pane[e,ImageSize->OptionValue@ImageSize,ImageSizeAction->"ShrinkToFit"]
        }],
    info=
      Replace[infoSpec,{
        (Rule|RuleDelayed)[k_,v_]:>
          {k,
            Replace[expr,{
              (a:{(_Rule|_RuleDelayed)..}|_Association)|
              _[a:{(_Rule|_RuleDelayed)..}|_Association]:>
                (Hold[v]/.a//ReleaseHold),
              _:>v
              }]
              }
        },      
    ...
    Interpretation[
      Framed[
            ...
            If[Length@info>0,
              {
                ...
                Style[
                  ...
                  FilterRules[{ops},
                      Alternatives@@DeleteCases[First/@Options@Style,
                        Background
                        ]
                    ],
                  FontColor->Gray
        ...
        FilterRules[{ops},Options@Framed],
...
IconForm~SetAttributes~HoldFirst;
...
(*{
EdgeForm[Directive[Thin,GrayLevel[.7]]],
GrayLevel[.95],
Rectangle[
{-2.5,-2.5},
{2.5,2.5}
...
Thread[
{
CirclePoints[{2.25,\[Pi]/3+\[Pi]/6},3],
CirclePoints[{1.2,\[Pi]/3+\[Pi]/6},3]
}
]}]
}]
...
CompressedSummaryForm[
  expr_,
  icon_:None]:=
  With[{c=Compress@expr},
    SummarizedForm[
      Unevaluated[CompressedData@c],
      "Compressed",
      icon,
      {
        Style[Head@expr,"Input"]
        },
      {
        "Expression: "->"expr",
        "Bytes: "->"byte_count",
      ...
      {
        "expr"->Style[HoldForm@expr,"Input"],
        "byte_count"->ByteCount@expr,
        "dimensions"->Dimensions@expr
        }
      ]
    ];
CompressedSummaryForm~SetAttributes~HoldFirst;


Options[CompressedIconForm]=Options@IconForm;
CompressedIconForm[expr_,icon_:None,ops:OptionsPattern[]]:=
  With[{
    c=Compress@expr,
    shortForm=Short[HoldForm@expr]},
    IconForm[
      CompressedData@c,
      icon,
      {
        {"Size: ",
          With[{bytes=ByteCount@c},
            Which[
              bytes>10^9,
                Quantity[Round[bytes/10^9,.01],"Gigabytes"],
              bytes>10^6,
                Quantity[Round[bytes/10^6,.01],"Megabytes"],
              bytes>10^3,
                Quantity[Round[bytes/10^3,.01],"Kilobytes"],
              True,
                Quantity[Round[bytes,.01],"Bytes"]
              ]
          ],SpanFromLeft},
        {"Expr: ",Short[HoldForm@expr]}
        },
      ops,
      Framed->False
      ]
    ];
CompressedIconForm~SetAttributes~HoldFirst;
...
CompressedForm[
    ...
    icon_:Automatic,
    args___,
    mode:"Summary"|"Icon":"Frame"]:=
  Switch[mode,
    "Box",
      CompressedSummaryForm[expr,
        Replace[icon,Automatic:>$CompressionIcon],args],
    "Frame",
      CompressedIconForm[expr,
        Replace[icon,Automatic:>$CompressionIcon],args]
    ];
CompressedForm~SetAttributes~HoldFirst;


StoreExpression[expr_,args___]:=
  With[{comp=CompressedForm[expr,Automatic,args]},
    CellPrint@
      Cell[
        BoxData@ToBoxes@Unevaluated[expr=comp;],
        "Input"  
        ]
    ];
StoreExpression~SetAttributes~HoldFirst
...
CompileNotebook[nb:_Notebook|_NotebookObject|Automatic:Automatic]:=
  Module[{
    compiledNotebook=
      Replace[nb,{
        Automatic:>NotebookGet@EvaluationNotebook[],
        _NotebookObject:>NotebookGet@nb
      }],
    styles,
    getsANDimports},
  styles=StylesheetNotebook@compiledNotebook;
  compiledNotebook=If[
    Quiet[Options[compiledNotebook,StyleDefinitions]]==={},
    Append[compiledNotebook,StyleDefinitions->styles],
    ReplaceAll[compiledNotebook,
      (StyleDefinitions->_)->(StyleDefinitions->styles)
      ]
    ];
...
GenerateNewPackage[name:_String:"Untitled Package"]:=
  CreateDocument@Notebook[{
    Cell[name,"CodeSection"],
    Cell[BoxData@"Begin[\"`Private`\"];","InputSection"],
    Cell["Exposed","Subsection",DefaultNewCellStyle->"CodeInput"],
    Cell["Constants","Subsubsection",DefaultNewCellStyle->"CodeInput"],
    Cell["Functions","Subsubsection",DefaultNewCellStyle->"CodeInput"],
    Cell["Formatting","Subsubsection",DefaultNewCellStyle->"CodeInput"],
    Cell["Internal","Subsection",DefaultNewCellStyle->"CodeInput"],
    Cell["Constants","Subsubsection",DefaultNewCellStyle->"CodeInput"],
    Cell["Functions","Subsubsection",DefaultNewCellStyle->"CodeInput"],
    Cell["Formatting","Subsubsection",DefaultNewCellStyle->"CodeInput"],
    Cell[BoxData@"End[];","InputSection"],
...
GenerateCodePackage[nb_Notebook]:=
  Insert[
    DeleteCases[
      nb/.{
        Cell[t__,
          s:"Chapter"|"Section"|"Subsection"|"Subsubsection",o__]:>
          Cell[t,"Code"<>s,o],
        Cell[e__,"Code",o___]:>
          Cell[e,"CodeInput",o]
        },
    ...
    StyleDefinitions->
      appFEFile["CodePackage.nb"],
    2
    ];
GenerateCodePackage[nb_NotebookObject]:=
  If[MatchQ[#,_Notebook],CreateDocument@#,$Failed]&@
    GenerateCodePackage[NotebookGet[nb]];
GenerateCodePackage[f_String?(FileExistsQ)]:=
  With[{nb=NotebookOpen[f,Visible->False]},
    (NotebookClose@nb;
      If[MatchQ[#,_Notebook],CreateDocument@#,$Failed]&@#)&@
      DeleteCases[GenerateCodePackage[NotebookGet@nb],Visible->_]
    ];
...
$CleanStylingRules={
"CodeSection"->{"Section",FrontEnd`InputGroup->True},
"CodeInput"->{"Input",AutoIndent->True,PageWidth->\[Infinity]},
"InputSection"->{"Input",CellGroupingRules->{"SectionGrouping",40}},
"Section"->{"Section",ShowGroupOpener->True}
};


CleanStyledNotebook[n_Notebook]:=
  n/.(Cell[e___,First@#,o___]->Cell[e,Sequence@@Last@#,o]&/@$CleanStylingRules)
...
Options[SetDockedCells]=Options@Cell;
SetDockedCells[
  ...
  SetOptions[
    ...
    DockedCells->Cell[BoxData@ToBoxes@expr,ops]
    ];
ClearDockedCells[nb:_NotebookObject|Automatic:Automatic]:=
  SetOptions[Replace[nb,Automatic:>EvaluationNotebook[]],DockedCells->None];


Options[EditDockedCells]=Options@Cell;
EditDockedCells[nb:_NotebookObject|Automatic:Automatic,
  function:Except[_Rule|_RuleDelayed]:Identity,
  ops:OptionsPattern[]]:=
  With[{notebook=Replace[nb,Automatic:>EvaluationNotebook[]]},
    SetOptions[notebook,
      DockedCells->Replace[
        (DockedCells/.Options[notebook,DockedCells])/.
          Cell[b_BoxData,o___]:>Cell[BoxData@ToBoxes@function@ToExpression@b,o],
        Cell[e:Except[_Rule|_RuleDelayed]...,op___]:>Cell[e,ops,op]
        ]
      ]
    ];


AvailableToolbars[args_,restrictKey_:"bar"]:=
  Cases[
    FEResourceFind["FEExpressions",args],
    r:(_String?(StringContainsQ[restrictKey])->_):>r
    ];
SetToolbar[nb:_NotebookObject|Automatic:Automatic,toolbarName_]:=
  With[{
    tb=AvailableToolbars@toolbarName,
    notebook=Replace[nb,Automatic:>EvaluationNotebook[]]
    },
    SetOptions[notebook,
      DockedCells->Replace[tb,{
        {}:>None,
        {_->b:Except[_Cell|_List],___}:>Cell@BoxData@b,
        {_->e_,___}:>e
        }]
        ]
    ];
...
SelectAbove[cellBase:_CellObject|Automatic:Automatic,breakTest_:None]:=
  With[{cell=Replace[cellBase,Automatic:>EvaluationCell[]]},
    With[{nb=ParentNotebook@cell,test=
      Replace[breakTest,{
            None->(#===cell&),
              s_String:>(MatchQ[NotebookRead[#],Cell[_,s,___]]&)
            }]},
      With[{cellChunks=SplitBy[Cells@nb,test]},
        With[{cells=
          Replace[
            FirstCase[cellChunks,_?(MemberQ[cell])],
            {cell,___}:>
              cellChunks[[First@First@Position[cellChunks,_?(MemberQ[cell])]-1]]
            ]},
            SelectionMove[cell,Before,Cell];
            Do[FrontEndTokenExecute[nb,"SelectPreviousLine"],Length@cells];
            (*SelectionMove[cell,Before,Cell];*)
          ]
        ]
      ]
...
EvaluateAbove[cellBase:_CellObject|Automatic:Automatic,breakTest_:None]:=
  With[{cell=Replace[cellBase,Automatic:>EvaluationCell[]]},
    SelectAbove[cell,breakTest];
    FrontEndTokenExecute[ParentNotebook@cell,"EvaluateCells"];
    SelectionMove[cellBase,After,Cell];
    ];


SelectBelow[cellObject_:Automatic,breakTest_:None]:=
  With[{cell=Replace[cellObject,Automatic:>EvaluationCell[]]},
    With[{nb=ParentNotebook@cell,test=
      Replace[breakTest,{
            None->(#===cell&),
            s_String:>(MatchQ[NotebookRead[#],Cell[_,s,___]]&)
      ...
      With[{cellChunks=SplitBy[Cells@nb,test]},
        With[{cells=
          Replace[FirstCase[cellChunks,_?(MemberQ[cell])],
              {___,cell}:>
                cellChunks[[First@First@Position[cellChunks,_?(MemberQ[cell])]+1]]
              ]},
          SelectionMove[cell,After,Cell];
          Do[FrontEndTokenExecute[nb,"SelectNextLine"],Length@cells];
          ]
        ]
      ]
    ];
EvaluateBelow[cell_:Automatic,breakTest_:None]:=
  With[{c=Replace[cell,Automatic:>EvaluationCell[]]},
    SelectBelow[c,breakTest];
    FrontEndTokenExecute[ParentNotebook@c,"EvaluateCells"];
    SelectionMove[cell,After,Cell];
    ];
...
NotebookSaveRename[
  f1:_NotebookObject|_String?FileExistsQ,
  f2_String,
  type:_String|Automatic:Automatic
  ]:=
  With[{
    nb=Replace[f1,_String:>NotebookOpen[f1,Visible->False]],
    t=Replace[type,
      Automatic:>
        Switch[FileExtension@f2,
          "nb",
            "Notebook",
          "m"|"wl",
            "Package",
...
lengthZeroQ[(_)[__]]:=False
lengthZeroQ[_]:=True;
lengthZeroQ~SetAttributes~HoldAllComplete;
...
With[{pythonAtomicExprs=pythonAtomicExprs},
  Format[pythonForm[code_,depth_:1]]:=
    Interpretation[
      TextCell[
      With[{tabination="\n"<>If[depth>0//TrueQ,
                        StringRepeat["\t",depth],
        ...
        Replace[Hold[code]/.{
                      ("\n"|"\t"|"\[IndentingNewLine]")->""
                      (*HoldPattern[h_[a___,CompoundExpression[e__],b___]]\[RuleDelayed]h[a,e,b]*)
                      },
        {
          a:Hold@pythonAtomicExprs:>ReplacePart[a,0->HoldForm],
          e_:>With[{eval=MapAt[pythonForm,ReplacePart[e,0->HoldForm],{1,2;;}]/.
                HoldPattern[pythonForm[c_]]:>Row@{tabination,pythonForm[c,depth+1]}},
                eval/;True]
          }]
        ](*/.(HoldPattern[(h:Except[Times|HoldForm|Set|SetDelayed])[e___]]\[RuleDelayed]h[Times[e]])*),
      "Output",
      Deployed->True],
      Defer@code];
    pythonForm~SetAttributes~HoldFirst
];
  ...
  CreateDocument[
    Import@
      PackageFilePath["Templates","PaletteTemplate.nb"]/.
            ...
            Cell[
              d_?(
                Not@FreeQ[
                  Flatten@{#},
                  _String?(
                    StringContainsQ["$PaletteTitle$"|"$PaletteName$"]
                    )]
                &),
              e__
              ]:>
        ...
        {
          s_String?(StringContainsQ["$PaletteTitle$"]):>
            StringReplace[s,
              "$PaletteTitle$"->title
              ],
          s_String?(StringContainsQ["$PaletteName$"]):>
            StringReplace[s,
              "$PaletteName$"->
                StringReplace[title,
